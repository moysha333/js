		const workSchedule = {
			mon: { opensAt: '08:01', closesAt: '17:45' },
			tue: 'closed',
		...
		function test() {
			var timeString = '2020-11-29T12:00:00';
		...
		function setTime(currDay, dayBefore) {
			if (currDay != 'closed' && dayBefore != 'closed') {
				...
				if (opensTime > closesTime && dbClosesTime < dbOpensTime) {
					if (opensTime <= now || dbClosesTime > now) {
						return true;
					}
				}
				else if (opensTime > closesTime && dbClosesTime > dbOpensTime) {
					if (opensTime <= now) {
						return true;
					}
				}
				else if (opensTime < closesTime && dbOpensTime > dbClosesTime) {
					if ((opensTime <= now && closesTime > now) || dbClosesTime > now) {
						return true;
					}
				}
				else if (opensTime < closesTime && dbOpensTime < dbClosesTime) {
					if(opensTime <= now && closesTime > now) {
						return true;
					}
				}
				else return false;
				...
				else if (opensTime <= now && closesTime > now) {
					return true;	
				}				
				else return false;			
			}
